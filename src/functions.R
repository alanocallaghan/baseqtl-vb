...
vb <- function(..., tol_rel_obj = 1e-2, n_iterations = 50000) {
    ...
    while (TRUE) {
        f <- try(rstan::vb(..., tol_rel_obj = tol_rel_obj, iter = n_iterations, output_samples = 4000))
        if (!inherits(f, "try-error")) {
            return(f)
        }
    }
...
fit_stan_noGT <- function(
    ...
    out <- lapply(
        ...
        function(condition) {
            ...
            rbias <- NULL
            if (is.null(model)) {
                model <- baseqtl:::stanmodels$noGT_nb
...
plot_with_legend_below <- function(...,
    ...
    if (common_x) {
        ...
        cowplot::plot_grid(
            together, xlab, legends[[1]],
            nrow = 3, rel_heights = rel_heights
        )
    } else {
        cowplot::plot_grid(
            together, legends[[1]],
            nrow = 2, rel_heights = rel_heights
        )
    }
}
