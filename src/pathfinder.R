...
pathfinder_parallel <- function(model, data, n = 4, mc.cores = getOption("mc.cores", 4), ...) {
  params <- names(rstan::optimizing(model, data, iter = 1)$par)
...
to_posterior <- function(model, data) {
  rstan::sampling(
    model,
    data = data, chains = 1, iter = 1, refresh = 0,
...
opt_path <- function(init, fn, gr,
  ...
  Ykt_h <- NULL
  Skt_h <- NULL # initialize matrics for storing history of updates
  t <- proc.time()
  for (iter in 1:Iter) {
    # cat(iter, "\t")
  ...
  if (N_sam > length(DIV_fit_pick$fn_draws)) {
    ...
    DIV_save$lp_approx_draws <- c(
      DIV_save$lp_approx_draws,
...
opt_path_stan <- function(model, data, init_bound = 2, N1 = 1000,
  ...
  out <- opt_path(
    init,
    fn = fn, gr = gr, N1 = N1, N_sam_DIV = N_sam_DIV,
...
opt_path_stan_parallel <- function(seed_init, seed_list, mc.cores, model, data,
  ...
  D <- get_num_upars(posterior)
  fn <- function(theta) {
    -log_prob(posterior, theta,
      adjust_transform = TRUE,
      gradient = TRUE
    )[1]
  }
  gr <- function(theta) {
    -grad_log_prob(posterior, theta,
      adjust_transform = TRUE
    )
  }
