## from https://github.com/LuZhangstat/Pathfinder/blob/bc19b9015c235f951c44e96146dab1194e1999f8/utils/sim_pf.R
pathfinder <- function(model, data, ...) {
    ## get param names
    params <- names(rstan::optimizing(model, data, iter = 1)$par)
    pfit <- opt_path_stan(
        model, data,
        N1 = 10000,
        N_sam = 1000,
    ...
    draws <- t(pfit$DIV_save$repeat_draws)
    colnames(draws) <- params
    draws
}

pathfinder_parallel <- function(
        model, data, n = 4, mc.cores = getOption("mc.cores", 4), ...
    ) {
    params <- names(rstan::optimizing(model, data, iter = 1)$par)
    fits <- opt_path_stan_parallel(
        seed_init = 1:n,
        seed_list = 1:n,
        mc.cores = mc.cores,
        model = model,
        data = data,
        init_bound = 2, N1 = 10000, N_sam_DIV = 1000, N_sam = 1000
        # , factr_tol = 1e5, lmm = 10
    )
...
to_posterior <- function(model, data) {
    rstan::sampling(
        model, data = data, chains = 1, iter = 1, refresh = 0,
...
opt_path <- function(
    ...
    while(LBFGS_fail) {   # if L-BFGS cannot run with the initial, find another inital
        ...
        if(tt$convcode == 9999) {
            LBFGS_fail <- TRUE
            splited_output <- unlist(lapply(my_data, function(x){
    ...
    if(is.null(DIV_ls) | (max(DIV_ls) == -Inf)) { # if no normal approximation generated, return the set of parameters found by optim
        return(
            list(
                sample_pkg_save = sample_pkg_save, # matrics for estiamted inv Hessian 
  ...
  return(
        list(
            sample_pkg_save = sample_pkg_save, 
            DIV_save = DIV_save, 
            y = y, 
            fn_call = fn_call, 
            gr_call = gr_call,
...
#' \code{gr_call}: no. calls of gradient
#' status     
opt_path_stan <- function(
        model, data, init_bound = 2, N1 = 1000,
        N_sam_DIV = 5, N_sam = 100,
        factr_tol = 1e2, lmm = 6, seed = 1234,
        eval_lp_draws = TRUE) {
    ...
    fn <- function(theta) -log_prob(
        posterior, theta, adjust_transform = TRUE, gradient = TRUE)[1] 
    gr <- function(theta) -grad_log_prob(
        posterior, theta, adjust_transform = TRUE)
    
    out <- opt_path(
        init, fn = fn, gr = gr, N1 = N1, N_sam_DIV = N_sam_DIV,
...
opt_path_stan_parallel <- function(
        seed_init, seed_list, mc.cores, model, data, 
...
#' @param eval_lp_draws  logical; if TRUE (default) evaluate the log-densities of approximating draws
#' @return A list of outputs from \code{\link{opt_path}}
opt_path_stan_init_parallel <- function(
        init_ls, mc.cores, model, data, 
        init_bound = 2.0, N1 = 1000, 
        N_sam_DIV = 5, N_sam = 100, 
        factr_tol = 1e2, lmm = 6,
    ...
    posterior <- to_posterior(model, data)
    D <- get_num_upars(posterior)
    fn <- function(theta) -log_prob(posterior, theta, adjust_transform = TRUE, gradient = TRUE)[1]
    ...
    out <- mclapply(list_ind,
        function(x){
            opt_path(
                init = init_ls[[x]], fn = fn, gr = gr, N1 = N1,
...
Form_N_apx_taylor <- function(x_l, g_l, Ykt_h, Skt_h, E, lmm){
    ...
    if ( 2*m >= D) {
        ...
        Mkbar <- rbind(
            cbind(matrix(0.0, nrow = m, ncol = m), diag(m)),
            cbind(diag(m), (diag(Dk, nrow = m) + tcrossprod(Ykt_h %*% diag(sqrt(E)))))
        )
        qrW <- qr(t(Wkbart))
        Qk <- qr.Q(qrW)
        Rkbar <- qr.R(qrW)
        ...
        x_center <- c(
            x_l # consider the first term in taylor expansion
...
Imp_Resam_WOR <- function(param_path, n_inits, seed = 123){
    ...
    check <- sapply(param_path, function(x){
    ...
    set.seed(seed)
    sample_ind <- sample.int(
        ncol(samples), size = n_inits, replace = FALSE, 
...
Imp_Resam_WR <- function(param_path, n_sam, seed = 123){
    ...
    set.seed(seed)
    sample_ind <-sample.int(
        ncol(samples), size = n_sam, replace = TRUE,
        prob = sample_weights_psis
    )
...
filter_samples_resam <- function(param_path, n_inits, seed = 123){
        ...
        for(indJ in 1:J){ #pathfinder
            if (indJ == indI){
                ...
                if (param_path[[indJ]]$sample_pkg_save$label == "full") {
                    ...
                    norm_u <- colSums(
                        forwardsolve(param_path[[indJ]]$sample_pkg_save$Rktilde, u2, transpose = TRUE, upper.tri = TRUE)^2) + 
                        colSums(u1^2) - colSums(u2^2)
                    lps_tem <- - param_path[[indJ]]$sample_pkg_save$logdetcholHk - 0.5 * norm_u
                }
    ...
    set.seed(seed)
    sample_ind <- sample(
        1:length(sample_weights), replace = TRUE,
...
get_opt_tr <- function(opath){
    ...
    for (l in 1:length(opath)) {
        ...
        ind_tr <- c(ind_tr, 1:nrow(opath[[l]]$y[, 1:(lp_ind - 1)]))
        tr_id <- c(tr_id, rep(l, nrow(opath[[l]]$y[, 1:(lp_ind - 1)])))
    }
...
Form_N_apx <- function(x_center, Ykt_h, Skt_h, E, lmm){
  ...
  if (2*m >= D) {
        ...
        sample_pkg <- list(
            ...
            Wkbart = Wkbart,
            x_center = x_center
        ) #inv_metric_est is returned for estimating mass matrix
    }
    return(sample_pkg)
}
